...
(ert-deftest herdr-schema-fixture-pins-critical-request-response-shapes ()
  ...
  (let* ((fixture (expand-file-name "fixtures/schema.json"
         ...
         (pane-split (funcall get "PaneSplitParams" defs))
         (tab-create (funcall get "TabCreateParams" defs))
         (workspace-create (funcall get "WorkspaceCreateParams" defs))
    ...
    (dolist (provisioning-schema (list pane-split tab-create workspace-create))
      (should (assoc 'env (funcall get "properties" provisioning-schema))))
    (should tab-created)
