  ...
  (let ((agent-fleet-dashboard-child-frame-fit-height t)
    ...
    (cl-letf* (((symbol-function 'frame-live-p) (lambda (_) t))
               ...
               ((symbol-function 'agent-fleet-list)
                (lambda () (make-list agents nil)))
               ((symbol-function 'frame-height) (lambda (_) -1))
               ((symbol-function 'frame-width) (lambda (_) 80))
               ((symbol-function 'set-frame-size)
                (lambda (_frame width height &rest _)
                  (setq resized (cons width height)))))
...
(ert-deftest agent-fleet-dashboard-child-frame-fit-height-clamps ()
  "Fit height is clamped to the configured min and max."
  (should (equal '(80 . 4)     ; 0 agents -> min
...
(ert-deftest agent-fleet-dashboard-child-frame-fit-height-is-off-by-default ()
  ...
  (let ((agent-fleet-dashboard-child-frame-fit-height nil)
        resized)
    (cl-letf* (((symbol-function 'frame-live-p) (lambda (_) t))
               ((symbol-function 'frame-parameter)
                (lambda (_f param)
...
(ert-deftest agent-fleet-dashboard-child-frame-fit-height-skips-non-child ()
  "Fit height only applies to the child-frame backend, not buffer/frame."
  (dolist (display '(buffer frame nil))
    (let ((agent-fleet-dashboard-child-frame-fit-height t)
          resized)
      (cl-letf* (((symbol-function 'frame-live-p) (lambda (_) t))
                 ((symbol-function 'frame-parameter)
                 ...
                 ((symbol-function 'set-frame-size)
                  (lambda (&rest _) (setq resized t))))
        (agent-fleet-dashboard--fit-child-frame-height 'dashboard))
      (should-not resized))))

(ert-deftest agent-fleet-dashboard-child-frame-fit-height-is-idempotent ()
  "A status-only event recomputes the same height and skips the resize."
  (let ((agent-fleet-dashboard-child-frame-fit-height t)
        (agent-fleet-dashboard-child-frame-min-height 4)
        (agent-fleet-dashboard-child-frame-max-height 40)
        resized)
