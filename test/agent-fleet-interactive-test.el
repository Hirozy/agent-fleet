...
(ert-deftest agent-fleet-interactive-herdr-start-default-session ()
  ...
  (let ((herdr--conn nil)
    ...
    (cl-letf (((symbol-function 'executable-find)
              ...
              ((symbol-function 'start-process)
               (lambda (name buffer program &rest args)
                 (setq started-args (list name buffer program args))
                 'fake-herdr-server-process))
              ((symbol-function 'set-process-query-on-exit-flag)
               (lambda (&rest _) nil))
              ((symbol-function 'set-process-sentinel)
               (lambda (&rest _) nil))
              ((symbol-function 'process-live-p)
               (lambda (process) (eq process 'fake-herdr-server-process)))
              ((symbol-function 'herdr-connect)
    ...
    (should (= 1 ping-count))
    (should (equal "/usr/local/bin/herdr" (nth 2 started-args)))
    (should (equal '("--session" "default" "server")
                   (nth 3 started-args)))
  ...
  (let ((herdr--conn nil)
    ...
    (cl-letf (((symbol-function 'executable-find)
              ...
              ((symbol-function 'start-process)
               (lambda (name buffer program &rest args)
                 (setq started-args (list name buffer program args))
                 'named-herdr-server-process))
              ((symbol-function 'set-process-query-on-exit-flag)
               (lambda (&rest _) nil))
              ((symbol-function 'set-process-sentinel)
               (lambda (&rest _) nil))
              ((symbol-function 'process-live-p)
               (lambda (process) (eq process 'named-herdr-server-process)))
              ((symbol-function 'herdr-connect)
               (lambda (path) (setq connected-path path) t)))
      (should (eq t (herdr-start))))
    (should (equal "/usr/local/bin/herdr" (nth 2 started-args)))
    (should (equal '("--session" "work" "server") (nth 3 started-args)))
    (should (equal (herdr-protocol--session-socket-path "work")
                   connected-path))))
...
(ert-deftest agent-fleet-interactive-herdr-start-cleans-early-exit ()
  "An early server exit keeps diagnostics and removes only the new process."
  (let ((herdr--conn nil)
        (herdr--server-processes nil)
        (herdr-default-session-name "default")
        (herdr-socket-path nil)
    ...
    (cl-letf (((symbol-function 'file-exists-p) (lambda (_) nil))
              ((symbol-function 'executable-find)
               (lambda (_) "/usr/local/bin/herdr"))
              ((symbol-function 'start-process)
               (lambda (_name buffer _program &rest _args)
                 (setq started-buffer buffer)
                 'dead-herdr-server-process))
              ((symbol-function 'set-process-query-on-exit-flag)
               (lambda (&rest _) nil))
              ((symbol-function 'set-process-sentinel)
               (lambda (&rest _) nil))
              ((symbol-function 'process-live-p) (lambda (_) nil))
              ((symbol-function 'process-exit-status) (lambda (_) 17))
              ((symbol-function 'delete-process)
               (lambda (process) (setq deleted process)))
              ((symbol-function 'herdr-connect)
               (lambda (&rest _) (error "must not connect"))))
      (should-error (herdr-start) :type 'user-error))
    (should (buffer-live-p started-buffer))
    (should (eq nil deleted))
    (should-not herdr--server-processes)))

(ert-deftest agent-fleet-interactive-herdr-start-times-out-and-stops-owned-process ()
  "A readiness timeout stops only the process started by this invocation."
  (let ((herdr--conn nil)
        (herdr--server-processes nil)
        (herdr-default-session-name "default")
        (herdr-socket-path nil)
    ...
    (cl-letf (((symbol-function 'file-exists-p) (lambda (_) nil))
              ((symbol-function 'executable-find)
               (lambda (_) "/usr/local/bin/herdr"))
              ((symbol-function 'start-process)
               (lambda (&rest _) 'live-herdr-server-process))
              ((symbol-function 'set-process-query-on-exit-flag)
               (lambda (&rest _) nil))
              ((symbol-function 'set-process-sentinel)
               (lambda (&rest _) nil))
              ...
              ((symbol-function 'herdr-connect)
               (lambda (&rest _) (error "must not connect"))))
      (should-error (herdr-start) :type 'user-error))
    (should (eq 'live-herdr-server-process deleted))
    (should-not herdr--server-processes)))
