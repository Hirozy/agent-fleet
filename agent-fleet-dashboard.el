...
is at least `agent-fleet-display-child-frame-minimum-emacs-version', and
...
(defun agent-fleet-dashboard--display-in-child-frame (buffer)
  ...
  (let* ((parent (agent-fleet-dashboard--child-parent-frame))
         (reason (agent-fleet-display--child-frame-unavailable-reason
                  parent)))
    (if reason
...
;;; --- Obsolete aliases for moved display functions -------------------

(define-obsolete-function-alias 'agent-fleet-dashboard-child-frame-available-p
  'agent-fleet-display-child-frame-available-p "0.7.0")
